...
describe("Matrix", () => {
  ...
  describe("numberMultiply", () => {
    it("returns matrix multiplied by number", () => {
      const matrix = new Matrix([new Vector([1, 1, 1])])
      const actual = matrix.numberMultiply(-1)
      const expected = new Matrix([new Vector([-1, -1, -1])])
      AssertUtils.assertMatrixEqual(actual, expected)
    })
  })
});
